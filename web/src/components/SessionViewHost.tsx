...
import type { BackgroundSummary, SessionResponse, WorkspaceRepoSummary } from "../lib/types";
...
/** The background summary is a fresh object on every poll, so the layer
 *  comparison has to look at its contents. */
function sameBackground(a: BackgroundSummary | undefined, b: BackgroundSummary | undefined): boolean {
  if (a === b) return true;
  if (!a || !b) return false;
  if (a.live !== b.live || a.lost_since !== b.lost_since || a.items.length !== b.items.length) return false;
  return a.items.every((item, i) => {
    ...
    return (
      ...
      item.ended?.reason === other.ended?.reason &&
      item.ended?.cause === other.ended?.cause &&
      item.ended?.at === other.ended?.at
    );
  });
}
