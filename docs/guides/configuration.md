...
| `acp.extra_fs_roots` | `[]` | Absolute directories every structured view session may read and write through ACP `fs/*` requests, on top of its own working directory, e.g. `["/home/me/shared-notes"]`. Matters for agents that delegate file access to aoe (grok): without it their file tools get "path is outside session roots" for anything outside the project. Agents that read files themselves are unaffected. Not applied to container-sandboxed sessions. Relative entries are ignored with a warning. Read from the global config only, like `acp.allowed_agents`, and changing the web value requires the passphrase step-up. Applies to a worker when it next starts or respawns. |
